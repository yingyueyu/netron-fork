...
import { tensorsConversion } from "./export/index.js";
...
view.View = class {
  constructor(host) {
    ...
    this._worker = this._host.environment("measure")
      ? null
      : new view.Worker(this._host);
  }
  ...
  async start() {
    try {
      ...
      this._element("toolbar-path-back-button").addEventListener(
        "click",
        async () => {
          await this.popGraph();
        }
      );
      this._element("sidebar").addEventListener(
        "mousewheel",
        (e) => {
      ...
      if (this._host.environment("menu")) {
        ...
        if (this._host.type === "Electron") {
          ...
          file.add({
            ...
            accelerator: "CmdOrCtrl+Shift+E",
            execute: async () =>
              await this.export(`${this._host.document.title}.png`),
            enabled: () => this.activeGraph,
          });
          file.add({
            label: "Export as &SVG",
            ...
            execute: async () =>
              await this.export(`${this._host.document.title}.svg`),
            enabled: () => this.activeGraph,
          });
        }
        ...
        view.add({
          label: () =>
            this.options.attributes ? "Hide &Attributes" : "Show &Attributes",
        ...
        view.add({
          label: () =>
            this.options.weights ? "Hide &Weights" : "Show &Weights",
        ...
        view.add({
          label: () =>
            this.options.mousewheel === "scroll"
              ? "&Mouse Wheel: Zoom"
              : "&Mouse Wheel: Scroll",
        ...
        if (
          this._host.type === "Electron" &&
          !this._host.environment("packaged")
        ) {
          view.add({});
          view.add({
            ...
            execute: async () =>
              await this._host.execute("toggle-developer-tools"),
          });
        }
      ...
      tensorsConversion(this._graph._tensors);
      debugger;
    } catch (error) {
  ...
  show(page) {
    ...
    this._host.document.body.classList.remove(
      ...Array.from(this._host.document.body.classList).filter(
        (_) => _ !== "active"
      )
    );
  ...
  find() {
    if (this._graph && this._sidebar.identifier !== "find") {
      this._graph.select(null);
      const sidebar = new view.FindSidebar(
        this,
        this._find,
        this.activeGraph,
  ...
  _activate() {
    ...
    if (this._host.environment("agent") === "safari") {
      ...
      graph.addEventListener("touchstart", this._events.touchstart, {
        passive: true,
      });
    }
  }
  ...
  _touchStartHandler(e) {
    ...
    const touchMoveHandler = (e) => {
      if (
        Array.isArray(this._touchPoints) &&
        this._touchPoints.length === 2 &&
        e.touches.length === 2
      ) {
  ...
  _scrollHandler(e) {
    if (
      this._scrollLeft &&
      e.target.scrollLeft !== Math.floor(this._scrollLeft)
    ) {
  ...
  scrollTo(selection, behavior) {
    if (selection && selection.length > 0) {
      ...
      if (
        ...
        const options = {};
        options.left =
          container.scrollLeft + x - bounds.left - bounds.width / 2;
  ...
  async error(error, name, screen) {
    ...
    const knowns = [
      {
        message: /^Invalid value identifier/,
        url: "https://github.com/lutzroeder/netron/issues/540",
      },
      {
        message: /^Cannot read property/,
        url: "https://github.com/lutzroeder/netron/issues/647",
      },
      {
        message: /^Duplicate value /,
        url: "https://github.com/lutzroeder/netron/issues/1364",
      },
      {
      ...
      {
        message: /^Invalid file content. File contains PaddlePaddle IR data./,
        url: "https://github.com/lutzroeder/netron/issues/1384",
      },
      {
      ...
      },
      {
        ...
        message:
          /^File format is not onnx\.ModelProto \(Unexpected end of file\)\./,
        url: "https://github.com/lutzroeder/netron/issues/1155",
      },
      {
        name: "Error loading ONNX model.",
        message:
          /^File format is not onnx\.ModelProto \(Cannot read properties of undefined \(reading 'ModelProto'\)\)\./,
        url: "https://github.com/lutzroeder/netron/issues/1156",
      },
      {
        name: "Error loading ONNX model.",
    ...
    const known = knowns.find(
      (known) =>
        (!known.name || known.name === error.name) &&
        error.message.match(known.message)
    );
    ...
    if (report) {
      this._host.openURL(
        url || `${this._host.environment("repository")}/issues`
      );
    }
    this.show(screen);
  }
    ...
    try {
      ...
      if (Array.isArray(model.graphs) && model.graphs.length > 0) {
        ...
        const entry = {
          graph,
          signature:
            Array.isArray(graph.signatures) && graph.signatures.length > 0
              ? graph.signatures[0]
              : null,
        };
        stack.push(entry);
      ...
      error.context =
        !error.context && context && context.identifier
          ? context.identifier
          : error.context || "";
      throw error;
    }
  ...
  async _updateGraph(model, stack) {
    const update = async (model, stack) => {
      this._model = model;
      this._stack = stack;
      const status = await this.renderGraph(
        this._model,
        this.activeGraph,
        this.activeSignature,
        this._options
        ...
        if (this._stack.length <= 1) {
          ...
          for (let i = count; i >= 0; i--) {
            ...
            if (name.length > 24) {
              ...
              element.innerHTML = `&hellip;${name.substring(
                name.length - 24,
                name.length
              )}`;
            } else {
  ...
  async renderGraph(model, graph, signature, options) {
    ...
    const background = this._host.document.createElementNS(
    ...
    const origin = this._host.document.createElementNS(
      "http://www.w3.org/2000/svg",
      "g"
    );
    ...
    if (status === "") {
      ...
      const elements = Array.from(
        canvas.getElementsByClassName("graph-input") || []
      );
      if (elements.length === 0) {
        const nodeElements = Array.from(
          canvas.getElementsByClassName("graph-node") || []
        );
      ...
      origin.setAttribute(
        "transform",
        `translate(${margin - size.x}, ${margin - size.y}) scale(1)`
      );
      ...
      this._zoom =
        this._stack && this._stack.length > 0 && this._stack[0].zoom
          ? this._stack[0].zoom
          : 1;
      this._updateZoom(this._zoom);
      const container = this._element("graph");
      const context =
        this._stack && this._stack.length > 0 && this._stack[0].context
          ? viewGraph.select([this._stack[0].context])
          : [];
      if (context.length > 0) {
  ...
  async export(file) {
    ...
    const extension =
      lastIndex === -1 ? "png" : file.substring(lastIndex + 1).toLowerCase();
    if (this.activeGraph && (extension === "png" || extension === "svg")) {
      ...
      origin.setAttribute(
        "transform",
        `translate(${delta - size.x}, ${delta - size.y}) scale(1)`
      );
      ...
      if (extension === "png") {
        try {
          const blob = await new Promise((resolve, reject) => {
            ...
            image.src = `data:image/svg+xml;base64,${this._host.window.btoa(
  ...
  showModelProperties() {
    if (this._model) {
      try {
        const sidebar = new view.ModelSidebar(
          this,
          this.model,
          this.activeGraph,
          this.activeSignature
  ...
  async showDefinition(type) {
    if (
      type &&
      (type.description || type.inputs || type.outputs || type.attributes)
    ) {
  ...
  about() {
    this._host.document.getElementById("version").innerText =
      this._host.version;
    const handler = () => {
...
view.Menu = class {
  constructor(host) {
    ...
    this._keydown = (e) => {
      this._alt = false;
      const code =
        e.keyCode | (e.altKey ? 0x0200 : 0) | (e.shiftKey ? 0x0100 : 0);
      const modifier = (e.ctrlKey ? 0x0400 : 0) | (e.metaKey ? 0x0800 : 0);
      if ((code | modifier) === 0x0212) {
        // Alt
        this._alt = true;
      } else {
        const action =
          this._accelerators.get(code | modifier) ||
          this._accelerators.get(
            code |
              ((e.ctrlKey && !this._darwin) || (e.metaKey && this._darwin)
                ? 0x1000
                : 0)
          );
    ...
    if (accelerator) {
      ...
      if (key !== "") {
        ...
        let code =
          ...
          (alt ? 0x0200 : 0) |
          (shift ? 0x0100 : 0);
        code |= this._keyCodes.has(key)
          ? this._keyCodes.get(key)
          : key.charCodeAt(0);
  ...
  unregister(action) {
    this._accelerators = new Map(
      Array.from(this._accelerators.entries()).filter(
        ([, value]) => value !== action
      )
    );
  }
  ...
  _mnemonic(code) {
    ...
    if (this._stack.length > 0 && key && (alt || !modifier)) {
      ...
      const group =
        this._stack.length > 0 ? this._stack[this._stack.length - 1] : this;
      const item = group.items.find(
        (item) =>
          key === item.mnemonic &&
          (item.type === "group" || item.type === "command") &&
          item.enabled
      );
      if (item) {
        return item;
    ...
    if (this._root.length > 1) {
      ...
      if (group) {
        const button = this._buttons.find(
          (button) => button.getAttribute("id") === group.identifier
        );
        if (button) {
          button.focus();
        }
    ...
    if (value) {
      ...
      if (index !== -1) {
        if (mnemonic) {
          ...
          return `${value.substring(0, index)}<u>${
            value[index + 1]
          }</u>${value.substring(index + 2)}`;
        }
  ...
  add(value) {
    const item =
      Object.keys(value).length > 0
        ? new view.Menu.Command(value)
        : new view.Menu.Separator();
...
view.Graph = class extends grapher.Graph {
  ...
  add(graph, signature) {
    ...
    this.identifier +=
      graph && graph.name ? `.${graph.name.replace(/\/|\\/, ".")}` : "";
    ...
    for (const node of graph.nodes) {
      ...
      if (
        Array.isArray(node.controlDependencies) &&
        node.controlDependencies.length > 0
      ) {
...
view.Node = class extends grapher.Node {
  constructor(context, value, type) {
    ...
    this.value = value;
    this.id = `node-${
      value.name ? `name-${value.name}` : `id-${context.counter++}`
    }`;
  ...
  _add(value, type) {
    ...
    if (
      (node.type && typeof node.type.name !== "string") ||
      !node.type.name.split
    ) {
      // #416
      const error = new view.Error(
        `Unsupported node type '${JSON.stringify(node.type.name)}'.`
      );
    ...
    let content =
      options.names && (node.name || node.identifier)
        ? node.name || node.identifier
        : node.type.name.split(".").pop();
    const tooltip =
      options.names && (node.name || node.identifier)
        ? node.type.name
        : node.name || node.identifier;
    if (content.length > 18) {
      content = `${content.substring(0, 9)}\u2026${content.substring(
        content.length - 9,
        content.length
      )}`;
    }
    const styles = category
      ? ["node-item-type", `node-item-type-${category.toLowerCase()}`]
      : ["node-item-type"];
    ...
    if (
      node.type.type ||
      (Array.isArray(node.type.nodes) && node.type.nodes.length > 0)
    ) {
      let icon = "\u0192";
      ...
      definition.on(
        "click",
        async () => await this.context.view.pushGraph(node.type, this.value)
      );
    }
    ...
    const attribute = (argument) => {
      let content = new view.Formatter(
        argument.value,
        argument.type
      ).toString();
    ...
    const isObject = (node) => {
      if (
        ...
        (node.type &&
          Array.isArray(node.type.nodes) &&
          node.type.nodes.length > 0)
      ) {
        return true;
      }
    ...
    if (Array.isArray(inputs)) {
      for (const argument of inputs) {
        ...
        if (
          ...
          (!argument.type || argument.type.endsWith("*")) &&
          argument.value.some((value) => value !== null && value.initializer)
        ) {
          hiddenTensors = true;
        } else if (
          options.attributes &&
    ...
    for (const argument of objects) {
      ...
      if (type === "graph") {
        ...
        content = argument.value.map((value) =>
          this.context.createGraph(value)
        );
  ...
  toggle() {
    ...
    this._graph = new view.Graph(
      this.context.view,
      this.context.view.host,
      this.context.model,
    ...
    if (this.from && Array.isArray(this.to)) {
      for (let i = 0; i < this.to.length; i++) {
        ...
        if (
          ...
          content = type.shape.dimensions
            .map((dim) =>
              dim !== null && dim !== undefined && dim !== -1 ? dim : "?"
            )
            .join("\u00D7");
...
view.Edge = class extends grapher.Edge {
  ...
  get minlen() {
    if (
      this.from.inputs.every(
        (argument) =>
          (!argument.type || argument.type.endsWith("*")) &&
          argument.value.every((value) => value.initializer)
      )
    ) {
      return 2;
    }
...
view.Sidebar = class {
  ...
  _update(stack) {
    ...
    this._host.document.removeEventListener(
      "keydown",
      this._closeSidebarKeyDownHandler
    );
    if (this._stack.length > 0) {
    ...
    if (this._stack.length > 0) {
      ...
      this._host.document.addEventListener(
        "keydown",
        this._closeSidebarKeyDownHandler
      );
...
view.Expander = class extends view.Control {
  ...
  toggle() {
    this._count =
      this._count === -1 ? this.element.childElementCount : this._count;
    if (this._expander) {
...
view.ValueView = class extends view.Expander {
  constructor(context, value, source) {
    ...
    try {
      ...
      this._hasCategory =
        initializer && initializer.category && source !== "attribute"
          ? true
          : false;
      if (
        this._hasId ||
        (!this._hasCategory && !type && source !== "attribute")
      ) {
        this._hasId = true;
        const element = this.createElement("div", "sidebar-item-value-line");
        if (typeof name !== "string") {
          throw new Error(
            `Invalid value identifier '${JSON.stringify(name)}'.`
          );
        }
        ...
        const line = this.createElement(
          "span",
          "sidebar-item-value-line-content"
        );
        line.innerText = "name: ";
  ...
  expand() {
    try {
      ...
      if (quantization) {
        ...
        if (
          quantization.type &&
          (quantization.type !== "linear" || (value && value !== "q"))
        ) {
          const line = this.createElement(
            "div",
            "sidebar-item-value-line-border"
...
view.TensorView = class extends view.Expander {
  ...
  get content() {
    ...
    if (
      ...
      const promise =
        value.peek && !value.peek() ? value.read() : Promise.resolve();
      promise.then(() => {
        if (tensor.empty) {
          ...
          if (
            ...
            this._saveButton = this.createElement(
              "div",
  ...
  async export() {
    ...
    if (file) {
      try {
        ...
        bytes.seek(0);
        const blob = new Blob([bytes.read()], {
...
view.NodeView = class extends view.Expander {
  constructor(context, node) {
    ...
    if (type) {
      ...
      const element = this.createElement("div", "sidebar-item-value-line");
      element.innerHTML = `<span class='sidebar-item-value-line-content'>node: <b>${
        type || " "
      }</b></span>`;
      element.addEventListener("pointerenter", () =>
        this.emit("focus", this._node)
      );
      element.addEventListener("pointerleave", () =>
        this.emit("blur", this._node)
      );
      element.addEventListener("click", () =>
        this.emit("activate", this._node)
      );
      element.style.cursor = "pointer";
      this.element.appendChild(element);
    } else {
      const element = this.createElement("div", "sidebar-item-value-line");
      element.innerHTML = `<span class='sidebar-item-value-line-content'>name: <b>${
        name || " "
      }</b></span>`;
      element.addEventListener("pointerenter", () =>
        this.emit("focus", this._node)
...
view.TensorSidebar = class extends view.ObjectSidebar {
  ...
  render() {
    ...
    const name =
      tensor && tensor.name ? tensor.name : value.name.split("\n")[0];
    if (name) {
      this.addProperty("name", name);
    }
    if (tensor) {
      ...
      if (type) {
        ...
        const shape =
          type.shape && Array.isArray(type.shape.dimensions)
            ? type.shape.dimensions.toString(", ")
            : "?";
    ...
    if (value.initializer) {
      const tensor = value.initializer;
      const promise =
        tensor.peek && !tensor.peek() ? tensor.read() : Promise.resolve();
      promise.then(() => {
...
view.ModelSidebar = class extends view.ObjectSidebar {
  ...
  render() {
    ...
    if (
      graph &&
      Array.isArray(graph.signatures) &&
      graph.signatures.length > 0
    ) {
      const entries = new Map();
      entries.set("", graph);
      ...
      const selector = new view.SelectView(
        this._view,
        entries,
        signature || graph
      );
      selector.on("change", (sender, data) =>
        this.emit("update-active-graph-signature", data)
      );
...
view.DocumentationSidebar = class extends view.Control {
  ...
  render() {
    if (!this.element) {
      ...
      if (
        Array.isArray(type.type_constraints) &&
        type.type_constraints.length > 0
      ) {
...
view.FindSidebar = class extends view.Control {
      ...
      for (const term of this._terms) {
        ...
        if (value.type.shape) {
          ...
          if (value.type.shape && Array.isArray(value.type.shape.dimensions)) {
            const dimensions = value.type.shape.dimensions.map((dimension) =>
  ...
  _node(node) {
    ...
    if (this._state.node) {
      ...
      if (
        (name && this._term(name)) ||
        (type && this._term(type)) ||
        (identifier && this._term(identifier))
      ) {
            ...
            for (const value of argument.value) {
              if (value !== null && value.initializer && this._value(value)) {
                ...
                if (value.name) {
                  ...
                  Array.isArray(argument.value) &&
                  argument.value.length === 1 &&
                  argument.name.indexOf(".") !== -1
                ) {
                  content = argument.name;
                } else if (
                  value.type &&
                  value.type.shape &&
                  Array.isArray(value.type.shape.dimensions) &&
                  value.type.shape.dimensions.length > 0
                ) {
                  content = `${value.type.shape.dimensions
                    .map((d) => (d !== null && d !== undefined ? d : "?"))
                    .join("\u00D7")}`;
                }
                if (content) {
  ...
  _update() {
    try {
      this._reset();
      const inputs = this._signature
        ? this._signature.inputs
        : this._graph.inputs;
      if (this._state.connection) {
        for (const input of inputs) {
          for (const value of input.value) {
      ...
      if (this._state.connection) {
        const outputs = this._signature
          ? this._signature.outputs
          : this._graph.inputs;
        for (const output of outputs) {
  ...
  render() {
    ...
    for (const [name, toggle] of Object.entries(this._toggles)) {
      ...
      toggle.element.setAttribute(
        "title",
        this._state[name] ? toggle.hide : toggle.show
      );
      ...
      toggle.checkbox.addEventListener("change", (e) => {
        ...
        toggle.element.setAttribute(
          "title",
          e.target.checked ? toggle.hide : toggle.show
        );
        this.emit("state-changed", this._state);
        this._update();
      });
  ...
  toString() {
    if (this.type === "linear" || /^quant\d\d?_.*$/.test(this.type)) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        content.push(
          length > 1 ? `${i.toString().padStart(size, " ")}: ${s}` : `${s}`
        );
      }
      return content.join("\n");
      ...
      return this.value
        .map(
          (value, index) => `${index.toString().padStart(size, " ")}: ${value}`
        )
        .join("\n");
      ...
      /^q\d_[01k]$/.test(this.type) ||
      /^iq\d_[xsnlm]+$/.test(this.type)
    ) {
      return "";
    }
...
view.Formatter = class {
  ...
  _format(value, type, quote) {
    ...
    switch (type) {
      ...
      case "function[]":
        return value
          ? value.map((item) => item.type.name).join(", ")
          : "(null)";
      case "type":
        return value ? value.toString() : "(null)";
      case "type[]":
    ...
    if (entries.length === 1) {
      ...
      list = entries.map(
        ([name, value]) => `${name}: ${this._format(value, null, true)}`
      );
    }
  ...
  static tensor(value) {
    ...
    if (
      ...
      type.shape &&
      type.shape.dimensions &&
      Array.isArray(type.shape.dimensions)
    ) {
      if (
        type.shape.dimensions.length === 0 &&
        (!value.peek || value.peek() === true)
      ) {
      ...
      const content = type.shape.dimensions
        .map((d) => (d !== null && d !== undefined ? d : "?"))
        .join("\u00D7");
      return `\u3008${content}\u3009`;
    }
...
markdown.Generator = class {
    ...
    while (source) {
      ...
      if (match) {
        ...
        if (header.length === align.length) {
          ...
          token.cells = token.cells.map((cell) =>
            this._splitCells(cell, token.header.length)
          );
      ...
      if (match) {
        ...
        tokens.push({
          type: "blockquote",
          text,
          tokens: this._tokenize(text, [], links, top),
        });
        continue;
      }
  ...
  _tokenizeInline(source, links, inLink, inRawBlock, prevChar) {
      ...
      while (maskedSource) {
        ...
        if (match) {
          if (links.has(match[0].slice(match[0].lastIndexOf("[") + 1, -1))) {
            maskedSource = `${maskedSource.slice(0, match.index)}[${"a".repeat(
              match[0].length - 2
    ...
    while (source) {
      ...
      if (match) {
        ...
        if (index > -1) {
          const length =
            (match[0].indexOf("!") === 0 ? 5 : 4) + match[1].length + index;
          match[2] = match[2].substring(0, index);
          match[0] = match[0].substring(0, length).trim();
          match[3] = "";
        }
        const title = (match[3] ? match[3].slice(1, -1) : "").replace(
          this._escapesRegExp,
          "$1"
        );
        const href = match[2]
        ...
        source = source.substring(match[0].length);
        if (token.type === "link") {
          token.tokens = this._tokenizeInline(
            token.text,
            links,
            true,
            inRawBlock,
      ...
      match =
        this._reflinkRegExp.exec(source) || this._nolinkRegExp.exec(source);
      if (match) {
      ...
      if (
        ...
        (!match[1] ||
          (match[1] &&
            (prevChar === "" || this._punctuationRegExp.exec(prevChar))))
      ) {
      ...
      if (
        ...
        const masked = maskedSource.slice(-1 * source.length);
        const endReg =
          match[0] === "*" ? this._emEndAstRegExp : this._emEndUndRegExp;
        endReg.lastIndex = 0;
        let cap = "";
        while ((match = endReg.exec(masked)) !== null) {
      ...
      if (match) {
        ...
        tokens.push({
          type: "text",
          text: inRawBlock ? match[0] : this._escape(match[0]),
        });
        continue;
      }
      throw new Error(`Unexpected '${source.charCodeAt(0)}'.`);
      ...
      switch (token.type) {
        ...
        case "table": {
          ...
          token.tokens.cells = token.cells.map((cell) =>
            cell.map((row) =>
              this._tokenizeInline(row, links, false, false, "")
            )
          );
          break;
        }
    ...
    while (tokens.length > 0) {
      ...
      switch (token.type) {
        ...
        case "heading": {
          ...
          html += `<h${level}">${this._renderInline(
            token.tokens
          )}</h${level}>\n`;
          continue;
        }
  ...
  _encode(content) {
    if (this._escapeTestRegExp.test(content)) {
      return content.replace(
        this._escapeReplaceRegExp,
        (ch) => this._escapeReplacementsMap[ch]
      );
    }
    return content;
  }
  ...
  _escape(content) {
    if (this._escapeTestNoEncodeRegExp.test(content)) {
      return content.replace(
        this._escapeReplaceNoEncodeRegExp,
        (ch) => this._escapeReplacementsMap[ch]
      );
    }
    return content;
...
metrics.Tensor = class {
  ...
  get metrics() {
    if (this._metrics === null) {
      ...
      if (
        size < 0x800000 &&
        (type.dataType.startsWith("float") ||
          type.dataType.startsWith("bfloat")) &&
        (!keys.has("sparsity") ||
          !keys.has("min") ||
          (!keys.has("max") && !keys.has("mean")) ||
          !keys.has("max") ||
          !keys.has("std"))
      ) {
        ...
        if (!keys.has("std")) {
          ...
          this._metrics.push(
            new metrics.Argument("std", Math.sqrt(variance / count))
          );
        }
        if (!keys.has("sparsity")) {
          this._metrics.push(
            new metrics.Argument(
              "sparsity",
              count > 0 ? zeros / count : 0,
              "percentage"
            )
          );
        }
...
view.Context = class {
  constructor(context, identifier, stream) {
    ...
    identifier =
      typeof identifier === "string" ? identifier : context.identifier;
    ...
    this._identifier =
      index === -1 ? identifier : identifier.substring(index + 1);
  }
    ...
    if (!this._content.has(type)) {
      ...
      if (stream) {
        ...
        const match = (buffer, signature) => {
          return (
            signature.length <= buffer.length &&
            buffer.every(
              (value, index) =>
                signature[index] === undefined || signature[index] === value
            )
          );
        };
        ...
        const skip =
          ...
          (type !== "hdf5" &&
            match(buffer, [0x89, 0x48, 0x44, 0x46, 0x0d, 0x0a, 0x1a, 0x0a])) || // \x89HDF\r\n\x1A\n
          Array.from(this._tags).some(
            ([key, value]) =>
              key !== "flatbuffers" && key !== "xml" && value.size > 0
          ) ||
          ...
          switch (type) {
            case "json": {
              try {
                const buffer = stream.peek(
                  Math.min(this.stream.length, 0x1000)
                );
                if (
                  stream.length < 0x7ffff000 &&
                  (buffer.length < 8 ||
                    String.fromCharCode.apply(null, buffer.slice(0, 8)) !==
                      "\x89HDF\r\n\x1A\n") &&
                  buffer.some(
                    (v) =>
                      v === 0x22 ||
            ...
            case "pkl": {
              ...
              if (unpickler) {
                    ...
                    switch (saved_id[0]) {
                      ...
                      default: {
                        throw new python.Error(
                          `Unsupported persistent load type '${saved_id[0]}'.`
                        );
                      }
                    }
                  }
                ...
                if (
                  Array.from(types).every(
                    (name) => !name.startsWith("__torch__.")
                  )
                ) {
                  ...
                  this._content.set(
                    type,
                    new view.Error(
                      "PyTorch standalone 'data.pkl' format not supported."
                    )
                  );
                }
              }
              ...
              try {
                ...
                if (archive) {
                  ...
                  if (entries.size === 1) {
                    ...
                    const name =
                      key === "" ? this.identifier.replace(/\.gz$/, "") : key;
            ...
            case "npz": {
              try {
                ...
                if (
                  ...
                  entries.size > 0 &&
                  Array.from(entries.keys()).every((name) =>
                    name.endsWith(".npy")
                  )
                ) {
                  const execution = new python.Execution();
    ...
    if (!this._tags.has(type)) {
      ...
      if (stream) {
        ...
        const skip =
          ...
          (Array.from(this._tags).some(
            ([key, value]) => key !== "flatbuffers" && value.size > 0
          ) &&
            type !== "pb+") ||
          Array.from(this._content.values()).some((obj) => obj !== undefined) ||
...
view.ModelFactoryService = class {
  constructor(host) {
    ...
    this.register(
      ...
      [
        ...
        /^events.out.tfevents./,
        /^.*group\d+-shard\d+of\d+(\.bin)?$/,
      ],
      [".zip"]
    );
    this.register("./tensorrt", [
      ".trt",
      ".trtmodel",
      ".engine",
      ".model",
    ...
    this.register(
      "./espresso",
      [".espresso.net", ".espresso.shape", ".espresso.weights"],
      [".mlmodelc"]
    );
  ...
  _unsupported(context) {
    ...
    const json = () => {
      ...
      if (obj) {
        const formats = [
          ...
          {
            name: "TPU-MLIR tensor location data",
            tags: ["file-line", "subnet_id", "core_id"],
          }, // https://github.com/sophgo/tpu-mlir/blob/master/lib/Dialect/Tpu/Transforms/Codegen/TensorLocation.cpp
          {
            name: "HTTP Archive data",
            tags: ["log.version", "log.creator", "log.entries"],
          }, // https://w3c.github.io/web-performance/specs/HAR/Overview.html
          { name: "Trace Event data", tags: ["traceEvents"] },
          { name: "Trace Event data", tags: ["[].pid", "[].ph"] },
          {
            name: "Diffusers configuration",
            tags: ["_class_name", "_diffusers_version"],
          },
          {
            name: "Transformers configuration",
            tags: ["architectures", "model_type"],
          }, // https://huggingface.co/docs/transformers/en/create_a_model
          {
            name: "Transformers generation configuration",
            tags: ["transformers_version"],
          },
          {
            name: "Transformers tokenizer configuration",
            tags: ["tokenizer_class"],
          },
          {
            name: "Transformers tokenizer configuration",
            tags: ["<|im_start|>"],
          ...
          {
            name: "Transformers preprocessor configuration",
            tags: [
              "crop_size",
              "do_center_crop",
              "image_mean",
          ...
          {
            name: "Tokenizers data",
            tags: ["version", "added_tokens", "model"],
          }, // https://github.com/huggingface/tokenizers/blob/main/tokenizers/src/tokenizer/serialization.rs
          { name: "Jupyter Notebook data", tags: ["cells", "nbformat"] },
          { name: "Kaggle credentials", tags: ["username", "key"] },
          {
            name: ".NET runtime configuration",
            tags: ["runtimeOptions.configProperties"],
          },
          {
            name: ".NET dependency manifest",
            tags: ["runtimeTarget", "targets", "libraries"],
          },
        ];
        const match = (obj, tag) => {
          if (tag.startsWith("[].")) {
        ...
        for (const format of formats) {
          if (format.tags.every((tag) => match(obj, tag))) {
            throw new view.Error(
              `Invalid file content. File contains ${format.name}.`
            );
          }
        }
      ...
      const formats = [
        {
          ...
          tags: ["entry", "entry.target_class"],
        },
        {
          name: "StringIntLabelMapProto data",
          tags: ["item", "item.id", "item.name"],
        },
        {
          name: "caffe.LabelMap data",
          tags: ["item", "item.name", "item.label"],
        },
        {
          name: "Triton Inference Server configuration",
          tags: ["input", "output", "name", "platform"],
        }, // https://github.com/triton-inference-server/common/blob/main/protobuf/model_config.proto
        {
          name: "Triton Inference Server configuration",
          tags: ["input", "output", "backend"],
        }, // https://github.com/triton-inference-server/common/blob/main/protobuf/model_config.proto
        {
          name: "TensorFlow OpList data",
          tags: ["op", "op.name", "op.input_arg"],
        },
        {
          name: "vitis.ai.proto.DpuModelParamList data",
          tags: ["model", "model.name", "model.kernel"],
        },
        {
          name: "object_detection.protos.DetectionModel data",
          tags: ["model", "model.ssd"],
        },
        {
          name: "object_detection.protos.DetectionModel data",
          tags: ["model", "model.faster_rcnn"],
        },
        {
    ...
    const pb = () => {
      ...
      if (Object.keys(tags).length > 0) {
        const formats = [
          ...
          {
            name: "chrome_browser_media.PreloadedData",
            tags: [[1, 2]],
            identifier: "preloaded_data.pb",
          }, // https://github.com/kiwibrowser/src/blob/86afd150b847c9dd6f9ad3faddee1a28b8c9b23b/chrome/browser/media/media_engagement_preload.proto#L9
          {
            name: "mind_ir.ModelProto",
            tags: [
              [1, 2],
              [2, 2],
              [5, 2],
          ...
          }, // https://github.com/mindspore-ai/mindspore/blob/master/mindspore/core/proto/mind_ir.proto
          {
            name: "mindspore.irpb.Checkpoint",
            tags: [
              [
                1,
                [
          ...
          }, // https://github.com/mindspore-ai/mindspore/blob/master/mindspore/ccsrc/utils/checkpoint.proto
          {
            name: "optimization_guide.proto.PageTopicsOverrideList data",
            tags: [
              [
                1,
                [
          ...
          }, // https://github.com/chromium/chromium/blob/main/components/optimization_guide/proto/page_topics_override_list.proto
          {
            name: "optimization_guide.proto.ModelInfo data",
            tags: [
              [1, 0],
              [2, 0],
              [4, 0],
          ...
          }, // https://github.com/chromium/chromium/blob/22b0d711657b451b61d50dd2e242b3c6e38e6ef5/components/optimization_guide/proto/models.proto#L80
          {
            name: "Hobot Dnn data",
            tags: [
              [1, 0],
          ...
          }, // https://github.com/HorizonRDK/hobot_dnn
          {
            name: "Hobot Dnn data",
            tags: [
              [1, 0],
        ...
        for (const format of formats) {
          if (
            match(tags, format.tags) &&
            (!format.identifier || identifier === context.identifier)
          ) {
            const error = new view.Error(
              `Invalid file content. File contains ${format.name}.`
            );
        ...
        const format = (tags) => {
          const content = Object.entries(tags).map(([key, value]) => {
            return `${key}:${
              Object(value) === value ? `{${format(value)}}` : value
            }`;
          });
          return content.join(",");
        };
    ...
    const xml = () => {
      ...
      if (document && document.documentElement) {
        ...
        throw new view.Error(
          `Unsupported XML content '${tags.keys().next().value}'.`
        );
      }
    };
    const hdf5 = () => {
      ...
      if (obj) {
        throw new view.Error(
          `Invalid file content. File contains HDF5 content.`
        );
      }
    };
    const unknown = () => {
  ...
  async _openContext(context) {
    const modules = this._filter(context).filter(
      (module) => module && module.length > 0
    );
    const errors = [];
    ...
    try {
      const rootFolder = (files) => {
        ...
        const rotate = (list) =>
          list.length === 0
            ? []
            : list[0].map((item, index) => list.map(at(index)));
      ...
      const files = Array.from(entries).filter(
        ([name]) =>
          !(
            name.endsWith("/") ||
      ...
      const filter = async (queue, entries) => {
          ...
          for (const module of modules) {
            ...
            if (context.type) {
              matches = matches.filter(
                (match) =>
                  !factory.filter || factory.filter(context, match.type)
              );
              if (
                matches.every(
                  (match) =>
                    !match.factory.filter ||
                    match.factory.filter(match, context.type)
                )
              ) {
        ...
        if (matches.length > 1) {
          ...
          throw new view.ArchiveError(
            `Archive contains multiple model files '${content}'.`
          );
        }
        if (matches.length > 0) {
      ...
      const queue = files.filter(
        ([name]) => name.substring(folder.length).indexOf("/") < 0
      );
  ...
  accept(identifier, size) {
    ...
    for (const extension of this._patterns) {
      if (
        (typeof extension === "string" && identifier.endsWith(extension)) ||
      ...
      const list = this._factories.filter(
        (entry) =>
          (typeof entry.extension === "string" &&
            identifier.endsWith(entry.extension)) ||
          (entry.extension instanceof RegExp &&
            entry.extension.test(identifier)) ||
  ...
  async _openSignature(context) {
    ...
    if (stream) {
      ...
      const entries = [
        ...
        {
          name: "V8 natives blob",
          value: /^./,
          identifier: /^natives_blob\.bin/,
        },
        ...
        {
          name: "Mathematica Notebook data",
          value:
            /^\(\*\sContent-type:\sapplication\/vnd\.wolfram\.mathematica\s\*\)/,
        },
        {
          name: "llama2.c checkpoint",
          value:
            /^..\x00\x00..\x00\x00..\x00\x00..\x00\x00..\x00\x00..\x00\x00..\x00\x00/,
          identifier: /^stories\d+[KM]\.bin/,
        },
        { name: "Cambricon model", value: /^\x7fMEF/ },
        { name: "Cambricon model", value: /^cambricon_offline/ },
        {
          name: "VNN model",
          value: /^\x2F\x4E\x00\x00.\x00\x00\x00/,
          identifier: /.vnnmodel$/,
        },
      ];
      /* eslint-enable no-control-regex */
      ...
      for (const entry of entries) {
        if (
          ...
          (!entry.identifier || context.identifier.match(entry.identifier))
        ) {
          throw new view.Error(
            `Invalid file content. File contains ${entry.name}.`
          );
        }
      }
    }
